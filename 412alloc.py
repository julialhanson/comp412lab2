...
def k_flag(k, filename):
    ...
    while curr.next != head.next:
        ...
        if curr.vr2 != INVALID and curr.nu2 == float('inf') and vr_to_pr[curr.vr2] != INVALID:
            free_pr(curr.pr2, available_prs)  
        # handling O3
        if curr.vr3 != INVALID:
        ...
        #     if vr_to_pr[i] != INVALID and i != pr_to_vr[vr_to_pr[i]]:
                #print("WRONG MISMATCH: " + str(i) + " maps to " + str(vr_to_pr[i]) + " which maps to " + str(pr_to_vr[vr_to_pr[i]]))
        curr = curr.next
